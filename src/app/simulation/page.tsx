...
export default function SimulationPage() {
  ...
  // Slider states
  const [irradiance, setIrradiance] = useState(50);
  const [forecastError, setForecastError] = useState(0);
  const [baseLoad, setBaseLoad] = useState(10);

  // Fault states
  const [smartMeterOffline, setSmartMeterOffline] = useState(false);
  const [evDisconnected, setEvDisconnected] = useState(false);
  const [inverterFault, setInverterFault] = useState(false);

  // Derived simulation factors (for display only — these drive visual feedback)
  const solarFactor = irradiance / 50; // 1.0 = normal (50%), 0 = none, 2.0 = max
  const baseLoadKw = (baseLoad / 10) * 0.5; // 0.5kW at 10, scales up
    ...
    <div className="space-y-6">
      ...
      {resetStatus === "success" && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-md text-sm text-emerald-800 font-medium">
          ✓ Simulation reset — Demo Household and schedules restored to deterministic baseline.
        </div>
      )}
      {resetStatus === "error" && (
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-3">
              ...
              <Slider
                value={[irradiance]}
                onValueChange={(v) => setIrradiance(Array.isArray(v) ? v[0] : v)}
                max={100}
                step={5}
              />
            ...
            <div className="space-y-3 pt-4">
              ...
              <Slider
                value={[forecastError]}
                onValueChange={(v) => setForecastError(Array.isArray(v) ? v[0] : v)}
                max={50}
                step={5}
              />
              <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-3">
              ...
              <Slider
                value={[baseLoad]}
                onValueChange={(v) => setBaseLoad(Array.isArray(v) ? v[0] : v)}
                max={50}
                step={1}
              />
              <p className="text-xs text-muted-foreground">
            ...
            <div className="border-t pt-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    setBaseLoad(prev => Math.min(50, prev + 20));
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between border-b pb-4">
              ...
              <Switch
                ...
                onCheckedChange={setSmartMeterOffline}
              />
            </div>
            ...
            <div className="flex items-center justify-between border-b pb-4">
              ...
              <Switch
                ...
                onCheckedChange={setEvDisconnected}
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={setInverterFault}
              />
            </div>
          </CardContent>
        </Card>
